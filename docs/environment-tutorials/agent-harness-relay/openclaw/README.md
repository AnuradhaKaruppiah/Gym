...
Relay-enabled OpenClaw currently emits 23 ATOF events and a 13-step ATIF
trajectory. The checked-in run denies external web tools, so the trace stays
focused on repository-local work. The ATIF has 4 same-step correlated tool
observations and no standalone tool observation steps.
